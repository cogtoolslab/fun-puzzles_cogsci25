...
% 1 - OPENING - some games are more fun than others
...
% In Study 1, we introduce visual and solution-based metrics puzzle complexity, and examine how these features predict which puzzles receive the most "likes" in an online dataset of 460 puzzles. In Study 2, we develop an experimental paradigm to measure participants' self-reported enjoyment immediately after attempting a puzzle. We also collected participants' puzzle-solving traces in order to assess the relative contribution of puzzle complexity, difficulty, and behavioral dynamics in predicting enjoyment ratings. We also measured participants' predictions about which puzzles will be more enjoyable from brief exposures, and analyzed how these rapid appraisals are impacted by puzzle-solving practice.

% Puzzle-solving performance can be evaluated in terms of both goal achievement (e.g., how many boxes are placed on goal tiles) and efficiency (e.g., the amount of time or steps taken). Critically, boxes can only be pushed, not pulled, so it is possible for a box to be “stuck” in corners. Learning to avoid these irreversible positions can greatly reduce planning complexity and improve the chances and speed of success.
% Thus, Sokoban is a useful domain to study how participants might go beyond perceptual heuristics to evaluate puzzles, and also to assess how puzzle-solving experience might drive changes in puzzle evaluation.
% 

%  Previous research has found that people reliably judge how fun simple two-player games will be, and that such judgments are partly explained by a model that simulates a few potential game plays (Zhang et al 2024 cogsci). We build on this work by asking how such judgments may change with domain-specific practice. Research on expertise suggests that domain knowledge, experience, and skill impact how people reason about tasks, but past work has mostly focused on problem categorization and difficulty judgments. We take a first step towards examining the role of experience in affective appraisals.
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/fig3_new.pdf}
    % \includegraphics[width=0.8\textwidth]{gleaning_figs/glean-combined_plot_illustrator.pdf}
...
%; 66 of these had all boxes already on goal states.
...
%We next excluded another 66 puzzles where all boxes were initially on goal states, and another 159 puzzles that could not be solved by the A* search algorithm within 1 million iterations.
%, as we could not obtain algorithmic model-based measures of puzzle difficulty. 
% 159 puzzles were additionally excluded as they were too complex to be solved by an A* search algorithm within 1 million iterations. 
...
% While interesting, we leave analyses of what makes certain puzzles polarizing for future work.
...
% A higher like rate was observed for smaller puzzles ($\rho = 0.097$, $t(440)=2.03$, $p=.043$) and puzzles with fewer boxes ($\rho = -0.21$, $t(440)=-4.53$, $p<.0001$), though it was not related to the proportion of empty tiles ($\rho = -0.055$, $t(440)=-1.15$, $p=.25$).  Puzzles that A* could solve in fewer iterations also had a higher like rate ($\rho = -0.093$, $t(440)=-1.96$, $p=.051$) \kz{Spearman's rank correlation between likerate and iterations: ($\rho =-0.23$, $S = 17752570$, $p<.0001$)}, suggesting an overall preference for simpler puzzles. Interestingly, like rate was higher for puzzles requiring more steps per box ($\rho = 0.18$, $t(440)=3.74$, $p=.00021$). 
...
% Since expectations about task difficulty interacts with task enjoyment \cite{}

% To being answering these questions, we designed a controlled experiment in which we can control collect full puzzle-attempt traces, and run more detailed analyses accounting for variation 
% We were therefore fundamentally limited in to ask questions about.
% \begin{itemize}
%     \item No control over participant experience - same person could have submitted multiple attempts, etc.
%     \item Enjoyment metric may not represent full variability -- total number of reactions was a very small percent 
%     \item Cannot link attempt to reaction -- surely reactions are a function of the puzzle-solving experience, not just static puzzle properties? We want to have participant-specific measures, e.g., number of restarts, optimality of plan, etc.
%     \item No information about participants -- surely familiarity and expertise matters
% \end{itemize}
...
%On their own, each of these explained less than 1\% of total variance (adjusted R-squared values from a linear mixed effects model with random participant and item intercepts). Instead, puzzle completion rates in the online corpus was the best predictor of both ratings, explaining about 5\% of total variance.
...
% \jc{Include descriptives? look at top and bottom puzzle for each condition/phase, and ask for what proportion of participants they were ranked as such?}
...
% Aggregating across participants, puzzle ELO scores were highly correlated with the proportion of comparisons the puzzle was selected in (). 
...
% In Study 1, we examined which features of puzzle predict enjoyment  a puzzle layout predicted enjoyment ..., \textit{Across} puzzles, Surface-level puzzle features may be informative about the puzzle experience. 
% In Study 2, we investigated the relative impact of visual features, solution complexity, and puzzle-solving experience on enjoyment ratings. We found that simple read outs of puzzle-solving behavior accounted for significant variability in enjoyment ratings \textit{within} puzzles and participants. Further, we found that self-reported puzzle enjoyment was highly reliable across participants. 

% Future work on how people will use enjoyment appraisals when selecting tasks. 
% Future: Manipulate goal contexts 
% Future: other kinds of appraisals, such as which puzzles 

%how people reason about past and future experiences
%experiences that are both fruitful and rewarding.

% In exploratory analyses, we found an intriguing effect of puzzle-solving practice on which puzzles people think seems more enjoyable. At posttest, the puzzles that people thought seemed more enjoyable was significantly predicted by the puzzles that other participants though seemed more difficult. 
...
%We gratefully acknowledge support from NSF CAREER Award \#2436199 (J.E.F.), NSF DRL \#2400471 (J.E.F, K.Z.), and awards from ONR Science of Autonomy (J.E.F, J.C), the Stanford Human-Centered AI Institute (HAI; K.Z.), Stanford Accelerator for Learning (SAL; J.C, J.E.F), and Stanford Institute for Research in the Social Sciences (IRiSS; K.Z.).
